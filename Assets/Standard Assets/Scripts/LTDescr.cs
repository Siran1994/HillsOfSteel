...
public class LTDescr
{
	...
	{
		return ((trans != null) ? ("name:" + trans.gameObject.name) : "gameObject:null") + " toggle:" + toggle.ToString() + " passed:" + passed + " time:" + time + " delay:" + delay + " direction:" + direction + " from:" + from + " to:" + to + " diff:" + diff + " type:" + type + " ease:" + easeType + " useEstimatedTime:" + useEstimatedTime.ToString() + " id:" + id + " hasInitiliazed:" + hasInitiliazed.ToString();
	}
	...
	public bool updateInternal()
	{
		...
		if (delay <= 0f && num != 0f)
		{
			...
			if ((num > 0f) ? (passed >= time) : (passed <= 0f))
			{
				...
				bool num2 = loopCount == 0 || loopType == LeanTweenType.once;
				if (!num2 && onCompleteOnRepeat && hasExtraOnCompletes)
				{
					callOnCompletes();
				}
				return num2;
