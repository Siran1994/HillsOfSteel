...
namespace I2.Loc
{
	public static class LocalizationManager
	{
		...
		{
			return GetTermTranslation(Term, IsRight2Left, 0, ignoreRTLnumbers: true, applyParameters: false, null);
		}

		public static string GetTermTranslation(string Term, bool FixForRTL)
		{
			return GetTermTranslation(Term, FixForRTL, 0, ignoreRTLnumbers: true, applyParameters: false, null);
		}

		public static string GetTermTranslation(string Term, bool FixForRTL, int maxLineLengthForRTL)
		{
			return GetTermTranslation(Term, FixForRTL, maxLineLengthForRTL, ignoreRTLnumbers: true, applyParameters: false, null);
		}

		public static string GetTermTranslation(string Term, bool FixForRTL, int maxLineLengthForRTL, bool ignoreRTLnumbers)
		{
			return GetTermTranslation(Term, FixForRTL, maxLineLengthForRTL, ignoreRTLnumbers, applyParameters: false, null);
		}
		...
		public static string GetTermTranslation(string Term, bool FixForRTL, int maxLineLengthForRTL, bool ignoreRTLnumbers, bool applyParameters, GameObject localParametersRoot)
		{
			...
			TryGetTermTranslation(Term, out Translation, FixForRTL, maxLineLengthForRTL, ignoreRTLnumbers, applyParameters, localParametersRoot);
			return Translation;
		}

		public static bool TryGetTermTranslation(string Term, out string Translation)
		{
			return TryGetTermTranslation(Term, out Translation, FixForRTL: false, 0, ignoreRTLnumbers: true, applyParameters: false, null);
		}

		public static bool TryGetTermTranslation(string Term, out string Translation, bool FixForRTL)
		{
			return TryGetTermTranslation(Term, out Translation, FixForRTL, 0, ignoreRTLnumbers: true, applyParameters: false, null);
		}
		...
		public static string ApplyRTLfix(string line, int maxCharacters, bool ignoreNumbers)
		{
			...
			{
				string str = "@@" + ((char)(ushort)(num2 + list.Count)).ToString() + "@@";
		...
		{
			Regex regex = new Regex(ignoreNumbers ? "(\\s|[^\\x00-\\xff])+" : "(\\s|[^\\x00-\\/:-\\xff])+");
			if (maxCharacters <= 0)
			{
		...
		internal static void ApplyLocalizationParams(ref string translation, GameObject root)
		{
			...
			MatchCollection matchCollection = new Regex("{\\[(.*?)\\]}").Matches(translation);
			int i = 0;
			for (int count = matchCollection.Count; i < count; i++)
			{
				Match match = matchCollection[i];
				string localizationParam = GetLocalizationParam(match.Groups[match.Groups.Count - 1].Value, root);
