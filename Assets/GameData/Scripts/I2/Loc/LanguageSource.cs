...
namespace I2.Loc
{
	...
	public class LanguageSource : MonoBehaviour
	{
		...
		public void Import_Google(bool ForceUpdate = false)
		{
			...
			string sourcePlayerPrefName = GetSourcePlayerPrefName();
			if (!ForceUpdate && GoogleUpdateFrequency != 0 && DateTime.TryParse(PlayerPrefs.GetString("LastGoogleUpdate_" + sourcePlayerPrefName, ""), out DateTime result))
			{
		...
		{
			if (TryGetTermTranslation(term, out string Translation))
			{
				return Translation;
			}
			return string.Empty;
